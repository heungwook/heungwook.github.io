...
- Why PDF and PostScript outputs
    ...
    - Let's assume we create a chart with 2x2-inch (600 dpi resultion). Drawing raster chart image needs to handle 1,200x1,200=1,440,000 pixels and each pixel contains 3 or 4 bytes of memory, this will cause slow and huge outputs. But, for vector outputs like PDF/PostScript, drawing lines/circles/stroke/fill commands just need few bytes and creating rasterized images that requires much computing power and memory will be done by PDF viewers(like Adobe Acrobat PDF Reader) or printers.
    - PostScript and PDF also support CMYK, printing color images requires CMYK color-space for precise colors.
...
- Creating 1,000 pages(A4, each page has 2 charts) outputs
